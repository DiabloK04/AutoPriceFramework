...
def searching_parameters():
    rf_search_grid = {
        'RF__n_estimators': randint(100, 1000),                 # Number of trees between 100 and 1000
        'RF__max_depth': [None] + list(np.arange(5,51,1)),      # The maximum depth of the tree
        'RF__min_samples_split': randint(2, 50),                # Random integers between 2 and 20
        'RF__min_samples_leaf': randint(1, 50),                 # Random integers between 1 and 20
        'RF__max_features':['sqrt', 'log2'] + list(np.arange(0.1, 1.1, 0.1)),
        'RF__ccp_alpha': uniform(0.0, 0.1),                     # Cost-complexity parameter
        }
    return rf_search_grid
#%% Function for developing Random Forest
def develop_RF(X, y, warranty):
    ...
    pipeline = Pipeline([
        ...
        ('RF', RandomForestRegressor(bootstrap=True, oob_score=True, random_state = SEED))
        ])
    ...
    rf_search_grid = searching_parameters()
    random_search = RandomizedSearchCV(pipeline, rf_search_grid, n_iter = n_ite, cv = k, n_jobs = -1, verbose = 0, random_state = SEED, scoring = 'neg_root_mean_squared_error')
...
def print_results(results):
    ...
    print(f"Number of trees: {best_params['RF__n_estimators']}")
    print(f"Maximum depth of trees: {best_params['RF__max_depth']}")
    print(f"Minimum no. of samples needed to split node: {best_params['RF__min_samples_split']}")
    print(f"Minimum no. of samples needed to be leaf node: {best_params['RF__min_samples_leaf']}")
    print(f"No. of features when looking for the best split: {best_params['RF__max_features']}")
    print(f"Cost-Complexity parameter alpha: {best_params['RF__ccp_alpha']:.4f}")
...
#%% Decision Tree
def get_tree(results, current_brand):
    rf_final = results['rf_final']
    feat_names = results['feature_names']
    tree = rf_final.estimators_[0]
    plt.figure(figsize=(30, 20))
    plot_tree(tree, feature_names=feat_names, filled=True, max_depth=3, impurity=False, fontsize=11)
    plt.title('Decision Tree Random Forest - '+str(current_brand))
    plt.show()
...
def plot_feature_importance(results, current_brand):
    ...
    plt.title('Feature Importances Random Forest - '+str(current_brand))
    plt.xlabel('%IncRMSE')
    plt.ylabel('Feature')
    plt.tight_layout()
    plt.grid(True)
    plt.savefig('Feature Importance/'+str(current_brand)+'_features_imp_rf')
    plt.close()
...
def get_partial_dependence(results, current_brand, warranty):
    ...
    for feat in feat_names:
        pd_values = partial_dependence(rf_final, transformed_X_test, feat)
        ...
        if len(partial_dependence_values) > 1:
            PartialDependenceDisplay.from_estimator(
                rf_final, transformed_X_test, [feat], grid_resolution=50)
            plt.title('Partial Dependence Plot Random Forest - '+str(current_brand))
            plt.ylabel('Predicted price (€)')
            plt.grid(True)
            plt.savefig('PDP/'+str(current_brand)+'_pdp_imp_rf_'+str(feat))
            plt.close()
...
def plot_lime(results, current_brand, obs_index, num_feat, warranty):
    ...
    plt.tight_layout()
    plt.grid(True)
    plt.savefig('LIME/Lime_'+str(current_brand)+'_rf')
    plt.close()
    return explanation
#%% Main function
def main(current_brand, warranty):
    ...
    results = develop_RF(X, y, warranty)
    ...
    print('-' * 15, 'Settings for RF model', '-' * 15)
    ...
    print(f"Total elapsed runtime Random Forest: {int(hours):02d}:{int(minutes):02d}:{int(seconds):02d}")
...
res_summary = pd.DataFrame([
    {
     ...
     'n_estimators': results['best_params']['RF__n_estimators'],
     'max_depth': results['best_params']['RF__max_depth'],
     'min_samples_split': results['best_params']['RF__min_samples_split'],
     'min_samples_leaf': results['best_params']['RF__min_samples_leaf'],
     'max_features': results['best_params']['RF__max_features'],
     'ccp_alpha': results['best_params']['RF__ccp_alpha'],
     'Seed': SEED,
...
if warranty is True:
    res_summary.to_csv('rf_model_warranty_results.csv', index=False)
    out_df.to_csv('rf_lime_warranty_outliers.csv', index=False)
else:
    res_summary.to_csv('rf_model_no_warranty_results.csv', index=False)