...
def searching_parameters(n_features):
    orf_search_grid = {
        'oRF__n_estimators': randint(100, 1000),                 # Number of trees between 100 and 1000
        'oRF__max_depth': [None] + list(np.arange(5,51,1)),      # The maximum depth of the tree
        'oRF__min_samples_split': randint(2, 50),                # Random integers between 2 and 20
        'oRF__min_samples_leaf': randint(1, 50),                 # Random integers between 1 and 20
        'oRF__max_features': ['sqrt', 'log2'] + list(np.arange(0.1, 1.1, 0.1)),
        'oRF__feature_combinations': randint(1,n_features)
        }
    return orf_search_grid
#%%
def develop_oRF(X, y, warranty):
    n_features = X.shape[1]
    
    # Categorical/Nummerical features
    ...
    pipeline = Pipeline([
        ...
        ('oRF', ObliqueRandomForestRegressor(bootstrap=True, oob_score=True, random_state = SEED))
        ])
    ...
    orf_search_grid = searching_parameters(n_features)
    random_search = RandomizedSearchCV(pipeline, orf_search_grid, n_iter = n_ite, cv = k, n_jobs = -1, verbose = 0, random_state = SEED, scoring = 'neg_root_mean_squared_error')
    ...
    # Pipeline for transforming the dataset and fitting it to the oRF
    model = Pipeline([
        ('preprocess',preprocess),
        ('oRF', ObliqueRandomForestRegressor(
            n_estimators = best_params['oRF__n_estimators'],
            max_depth = best_params['oRF__max_depth'],
            min_samples_split = best_params['oRF__min_samples_split'],
            min_samples_leaf = best_params['oRF__min_samples_leaf'],
            max_features = best_params['oRF__max_features'],
            random_state = SEED,
            bootstrap = True,
            oob_score = True,
            feature_combinations = best_params['oRF__feature_combinations']
            ))
        ])
    
    # Retrain the model on training set
    model.fit(X_train, y_train)
    orf_final = model.named_steps['oRF']
...
def print_results(results):
    ...
    print(f"Number of trees: {best_params['oRF__n_estimators']}")
    print(f"Maximum depth of trees: {best_params['oRF__max_depth']}")
    print(f"Minimum no. of samples needed to split node: {best_params['oRF__min_samples_split']}")
    print(f"Minimum no. of samples needed to be leaf node: {best_params['oRF__min_samples_leaf']}")
    print(f"No. of features when looking for the best split: {best_params['oRF__max_features']}")
    print(f"No. of features to combine at each split: {best_params['oRF__feature_combinations']}")
...
def plot_feature_importance(results, current_brand):
    ...
    plt.title('Feature Importances Oblique Random Forest - '+str(current_brand))
    plt.xlabel('%IncRMSE')
    plt.ylabel('Feature')
    plt.tight_layout()
    plt.grid(True)
    plt.savefig('Feature Importance/'+str(current_brand)+'_features_imp_orf')
    plt.close()
#%% Partial Dependence Plots
def get_partial_dependence(results, current_brand, warranty):
    X_test = results['test_data']
    model = results['final_model']
    orf_final = results['orf_final']
    ...
    for feat in feat_names:
        pd_values = partial_dependence(orf_final, transformed_X_test, feat)
        ...
        if len(partial_dependence_values) > 1:
            PartialDependenceDisplay.from_estimator(
                orf_final, transformed_X_test, [feat], grid_resolution=50)
            plt.title('Partial Dependence Plot Oblique Random Forest - '+str(current_brand))
            plt.ylabel('Predicted price (€)')
            plt.savefig('PDP/'+str(current_brand)+'_pdp_imp_orf_'+str(feat))
            plt.close()
...
def plot_lime(results, current_brand, obs_index, num_feat, warranty):
    ...
    plt.tight_layout()
    plt.grid(True)
    plt.savefig('LIME/Lime_'+str(current_brand)+'_orf')
    plt.close()
    return explanation
#%% Main function
def main(current_brand, warranty):
    ...
    # Running time for oRF
    start_time = time.time()
    results = develop_oRF(X, y, warranty)
    ...
    print('-' * 15, 'Settings for oRF model', '-' * 15)                                      
    ...
    print(f"Total elapsed runtime Oblique Random Forest: {int(hours):02d}:{int(minutes):02d}:{int(seconds):02d}")
...
warranty=True # True if model with BERT warranty, False if the basic model
...
res_summary = pd.DataFrame([
    {
     ...
     'n_estimators': results['best_params']['oRF__n_estimators'],
     'max_depth': results['best_params']['oRF__max_depth'],
     'min_samples_split': results['best_params']['oRF__min_samples_split'],
     'min_samples_leaf': results['best_params']['oRF__min_samples_leaf'],
     'max_features': results['best_params']['oRF__max_features'],
     'feature_combinations': results['best_params']['oRF__feature_combinations'],
     'Seed': SEED,
...
if warranty is True:
    res_summary.to_csv('orf_model_warranty_results.csv', index=False)
    out_df.to_csv('orf_lime_warranty_outliers.csv', index=False)
else:
    res_summary.to_csv('orf_model_no_warranty_results.csv', index=False)